...
public class HexAndStringConverter {
	 ...
	 * @return
	 */
	public static byte[] convertHexStringToHexByte(String s) throws Exception{
		try{
		  int len = s.length();
		  byte[] data = new byte[len / 2];
		  for (int i = 0; i < len; i += 2) {
			  if( Character.digit(s.charAt(i),16)>15 || Character.digit(s.charAt(i),16)<0 )
				  throw new Exception("not 0~F byte format!");
			data[i / 2] = (byte) ((Character.digit(s.charAt(i), 16) << 4) + Character.digit(s.charAt(i + 1), 16));
		  }
		  return data;
		}catch(Exception ee){
			ee.printStackTrace();
			throw new Exception("not byte stirng fromat" );
		}
	}
     ...
     * @return
     */
    public static boolean  checkBlueToothAddressBetweenStartAndEnd(String sSrcMac,String sStartMac,String sEndMac) throws Exception{
